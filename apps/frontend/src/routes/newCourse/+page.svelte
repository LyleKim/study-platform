<script lang="ts">
    ...
    async function courseAddClick() {
        ...
        const res = await client.saveCourseInfo.$post({json:{ courseInfo }});
        let data = await res.json();
        inputCourseId = data[0].courseId;
        goto(`/coursePage/${inputCourseId}/`); // '/couser/[id]/+page.svelte'로 이동(dummyPage)
    }
